...
import { AddTutorial, RemoveTutorial, EditTutorial } from './../actions/tutorial.actions'
...
export class TutorialState {
    ...
    @Action(EditTutorial)
    edit({getState, patchState }: StateContext<TutorialStateModel>, { payload }:EditTutorial) {
        const state = getState();
        patchState({
            tutorials: getState().tutorials.filter(a => a.name != payload)
        })
